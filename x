...
local function getPredictedPosition(character)
    ...
    if primaryPart then
        ...
        local timeToPredict = getgenv().TriggerBot.Settings.Prediction.TimeToPredict or 0.08
        local predictionPower = getgenv().TriggerBot.Settings.Prediction.Power or 2
        
        -- Apply prediction power to adjust velocity impact
        local adjustedVelocity = velocity * predictionPower

        local predictedPosition = primaryPart.Position + (adjustedVelocity * timeToPredict)
...
local function TriggerBotAction()
    if TargetPlayer and TargetPlayer.Character then
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            ...
            if getgenv().TriggerBot.Settings.Prediction.EnableLegitPrediction then
            ...
            if predictedPosition and isWithinBoxFOV(predictedPosition) then
                ...
                if closestPart then
                    ...
                    if distance <= getgenv().TriggerBot.Settings.FiringDistance then
                        local currentDelay = math.random(getgenv().TriggerBot.Settings.TriggerDelay.Min, getgenv().TriggerBot.Settings.TriggerDelay.Max)
                        if os.clock() - lastClickTime >= currentDelay then
